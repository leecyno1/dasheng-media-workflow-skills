...
- `core`: Dasheng workflow skills that are directly called by intake, draft,
...
- `external-candidate`: Useful upstream skills discovered during research but not
  yet wired into the Dasheng workflow.
- `archive`: Historical Dasheng skills kept for reference only.
...
- Anthropic financial-service examples: `anthropic-fs-*`; keep as references
  unless a concrete Dasheng stage uses them.

...
| 编号 | 截图 Skill / 项目 | 已确认仓库 | GitHub 状态 | 与当前大圣链路关系 | 建议 |
| --- | --- | --- | --- | --- | --- |
...
发布依赖源码和虚拟环境放在 `vendor/publish/`；登录态放在 `~/Library/Application Support/DashengPublishSessions/` 或 `DashengPublishProfiles/`；第 7 环节生成的发布包、回执、截图和失败记录放在 `~/Desktop/自媒体创作/07_发布执行/`。三者不得混放。

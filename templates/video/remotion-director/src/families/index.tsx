...
const SourceTag: React.FC<{text?: string}> = ({text}) => {
  if (!text) return null;
  return (
    <div style={{position: 'absolute', left: 72, bottom: 42, fontSize: 17, color: P.muted, letterSpacing: '0.04em'}}>
      来源：{text}
    </div>
  );
};
  ...
  return (
    <>
      ...
      <div style={{position: 'absolute', left: 165, right: safeRight, top: 230, height: 690, borderRadius: 28, overflow: 'hidden', background: '#fff', boxShadow: '0 30px 90px rgba(20,33,29,.22)', transform: `translateY(${(1 - p) * 30}px)`}}>
        {scene.visual?.document_src ? <Img src={staticFile(scene.visual.document_src)} style={{width: '100%', height: '100%', objectFit: 'cover', objectPosition: '50% 35%'}} /> : <div style={{padding: 70}}><div style={{fontFamily: 'Georgia, serif', fontSize: 28, color: P.muted}}>OFFICIAL DOCUMENT / DEMO</div><div style={{fontFamily: 'Georgia, serif', fontSize: 58, fontWeight: 800, marginTop: 24}}>{scene.visual?.document_title ?? 'Exact source region'}</div><div style={{height: 2, background: P.line, margin: '36px 0'}} />{[0, 1, 2, 3, 4].map((row) => <div key={row} style={{height: 34, width: `${88 - row * 7}%`, background: row === 2 ? 'rgba(13,118,110,.16)' : '#e7e9e5', marginBottom: 20, borderRadius: 8}} />)}</div>}
        <div style={{position: 'absolute', left: 120, right: 150, top: 335, height: 105, border: `5px solid ${P.coral}`, borderRadius: 18, opacity: focus, boxShadow: '0 0 0 999px rgba(20,33,29,.18)'}} />
        <div style={{position: 'absolute', left: 120, top: 280, padding: '10px 16px', borderRadius: 12, background: P.coral, color: 'white', fontSize: 20, fontWeight: 850, opacity: documentLateCue, transform: `translateY(${(1 - documentLateCue) * 18}px)`}}>定位到当前命题的关键区域</div>
      ...
      <div style={{position: 'absolute', left: 110, right: safeRight, top: 380, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 20}}>
        {nodes.map((node, index) => {
          ...
          return <React.Fragment key={node}>
            <div style={{width: 300, minHeight: 150, padding: 28, borderRadius: 28, display: 'grid', placeItems: 'center', textAlign: 'center', background: index === nodes.length - 1 ? P.green : '#fff', color: index === nodes.length - 1 ? 'white' : P.ink, border: `2px solid ${index === nodes.length - 1 ? P.green : P.line}`, boxShadow: index === nodes.length - 1 ? `0 0 ${50 * conclusionLock}px rgba(13,118,110,.38)` : undefined, fontSize: 31, fontWeight: 850, transform: `translateY(${(1 - p) * 36}px)`, opacity: p}}>{node}</div>
            {index < nodes.length - 1 ? <svg width="120" height="60" viewBox="0 0 120 60"><path d="M4 30 H104" stroke={P.coral} strokeWidth={6} strokeLinecap="round" pathLength={1} strokeDasharray={1} strokeDashoffset={1 - p} /><path d="M88 14 L108 30 L88 46" fill="none" stroke={P.coral} strokeWidth={6} strokeLinecap="round" strokeLinejoin="round" opacity={p} /></svg> : null}
          </React.Fragment>;
        })}
      </div>
...
export const BrollFullscreenFamily: React.FC<FamilyProps> = ({scene, motionBehavior}) => {
  ...
  return (
    <>
      ...
      <div style={{position: 'absolute', left: 90, top: 260, width: 850, color: 'white', opacity: reveal, transform: `translateY(${(1 - reveal) * 30}px)`}}><div style={{fontSize: 18, letterSpacing: '0.17em', fontWeight: 800}}>{scene.visual?.eyebrow || '现场画面'}</div><div style={{fontFamily: 'Iowan Old Style, Songti SC, serif', fontSize: 72, fontWeight: 850, lineHeight: 1.05, marginTop: 18}}>{scene.title}</div><div style={{fontSize: 26, lineHeight: 1.5, marginTop: 24, maxWidth: 720}}>{scene.visual?.context ?? ''}</div></div>
      <SourceTag text={scene.visual?.source} />
    </>
  );
};

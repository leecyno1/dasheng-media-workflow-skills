...
def test_vox_storyboard_is_question_led_and_evidence_bounded(tmp_path):
    ...
    assert all(len(scene["micro_shots"]) >= 3 for scene in storyboard["scenes"])
    assert all(scene["micro_shots"][0]["continuity_anchor"] == "shared_paper_evidence_world" for scene in storyboard["scenes"])
    assert all(scene["image2_scene_policy"]["mode"] == "one_complete_scene_still_per_micro_shot" for scene in storyboard["scenes"])
    ...
    assert all(all(shot["shot_size"] and shot["focus"] and shot["crop_scale"] for shot in scene["micro_shots"]) for scene in storyboard["scenes"])

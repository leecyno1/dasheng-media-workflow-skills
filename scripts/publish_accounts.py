...
DEFAULT_SESSION_ROOT = Path.home() / "Library" / "Application Support" / "DashengPublishSessions"
...
def registry_path() -> Path:
    return Path(os.environ.get("DASHENG_PUBLISH_ACCOUNT_REGISTRY") or DEFAULT_REGISTRY).expanduser()


def browser_profile_path() -> Path:
    return Path(os.environ.get("DASHENG_PUBLISH_BROWSER_PROFILES") or DEFAULT_BROWSER_PROFILES).expanduser()
...
def session_root(registry: dict[str, Any]) -> Path:
    configured = os.environ.get("DASHENG_PUBLISH_SESSION_ROOT")
    if not configured:
        configured = str((registry.get("policy") or {}).get("session_root") or DEFAULT_SESSION_ROOT)

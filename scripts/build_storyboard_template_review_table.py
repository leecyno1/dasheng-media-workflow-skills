...
def review_page_script(
  ...
  <script>
    ...
    function getSceneDecision(scene) {{
      const row = document.querySelector(`[data-scene-id="${{CSS.escape(scene.scene_id)}}"]`);
    ...
    function buildDecisionPayload() {{
      ...
      const blockers = decisions.filter(item => !['approved'].includes(item.decision));
      return {{
    ...
    function downloadDecision() {{
      ...
      a.download = 'storyboard_review_decision.json';
      a.click();

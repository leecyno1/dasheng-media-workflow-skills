...
def convert(storyboard: dict[str, Any], inventory: dict[str, Any], review_gate: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for idx, scene in enumerate(storyboard.get("scenes", []), 1):
        ...
        item = {
            "id": scene.get("scene_id") or f"director_scene_{idx:03d}",
            ...
            "narration": scene.get("voiceover_text") or scene.get("narration"),
            "duration_sec": duration,
            "start_sec": start,
            "end_sec": float(scene.get("end_sec") or start + duration),
            "timing": {
                "char_count": len(str(scene.get("voiceover_text") or "")),
                "target_cps": 5.2,
    ...
    return {
        ...
        "aspect": storyboard.get("aspect", "9:16"),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Convert Dasheng director storyboard into renderable video timeline.")

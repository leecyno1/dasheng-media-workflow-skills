...
| `transwrite` | 公众号、口播视频、播客转写生产 | `dasheng-media-sop` / `dasheng-stage-transwrite` | `scripts/build_stage4_transwrite.py`、`dasheng-style-profiler`、`baoyu-cover-image`、`dasheng-video-style-trainer`、`dasheng-lemon-illustrations`、`video-hyperframes`、`remotion-best-practices` | `04_转写计划.md` / `transwrite_manifest.json` / lane `illustration_intents.json` / lane manifests |

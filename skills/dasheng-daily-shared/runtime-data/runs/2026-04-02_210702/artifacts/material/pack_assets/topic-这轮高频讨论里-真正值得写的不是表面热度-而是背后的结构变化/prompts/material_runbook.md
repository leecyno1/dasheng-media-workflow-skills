# 这轮高频讨论里，真正值得写的不是表面热度，而是背后的结构变化 素材执行手册

- 主题类型：`general_commentary`
...
4. AI 图像至少补齐连环画 10 张，梗图 3 张，搞笑人物 3 张。
...
- 推荐主链命令：`/Volumes/PSSD/Projects/公众号文章/scripts/material_execute_pack.sh --draft-manifest /path/to/draft_manifest.json --topic-dir topic-这轮高频讨论里-真正值得写的不是表面热度-而是背后的结构变化 --steps charts,image_search,video_search,ai_prep`
- 兼容命令：`/Volumes/PSSD/Projects/公众号文章/scripts/material_execute_pack.sh --pack-root /path/to/pack_assets --topic-dir topic-这轮高频讨论里-真正值得写的不是表面热度-而是背后的结构变化 --steps charts,image_search,video_search,ai_prep`

...
**关于配图**：写作时先保证内容成立，不要为了配图硬造比喻。正文完成后由 Draft 资产环节统一分析插图：真实数据走图表/证据图；原文中有认知价值的比喻、举例、类比、拟人或抽象机制时，生成 `illustration_intents` 并调用 `dasheng-lemon-illustrations` 制作柠檬人漫画，紧跟对应段落插入。

...
function renderTopicRunbook(plan) {
  return [
    ...
    '## 执行命令',
    '',
    `- 推荐主链命令：\`scripts/material_execute_pack.sh --draft-manifest ${draftManifestForRunbook()} --topic-dir ${plan.topic_slug} --steps charts,image_search,video_search,ai_prep\``,
    `- 正式命令：\`python3 scripts/material_execute_pack.py --draft-manifest <draft_manifest.json>\``,
    ''
  ].join('\n');
}

...
补充能力：`Paradigm Profile 范式学习` 作为阶段 0.5 的可选预处理资产，不单列为正式主链阶段。用户可在正式生产前提供标准文章、爆款样本、渠道模板或内部内容模板，由系统提炼“文章范式与框架”，再按场景、渠道、风格注入 Brief / Draft / Rewrite / Publish 的约束中。
...
- 第 3 阶段可继承 `Paradigm Profile` 中的结构范式，但仍然只生成事实基线稿，不做作者口吻或平台腔模仿。
...
- **作用到 Rewrite**：在已有事实终稿上叠加风格 DNA、节奏模板和版本策略，生成不同作者感、渠道感或场景感的变体。
- **作用到 Publish**：把同一内容按公众号、小红书、短视频脚本、社群帖等渠道拆成不同呈现框架。
- **不建议放在 Material**：素材阶段只补证据、图表、引用和资产，不应改变文章范式。

推荐对象链补充为：

`Run -> ParadigmProfile(optional) -> TopicPool -> SelectedTopic -> Draft -> MaterialPack -> RewritePack -> PublishPack -> Postmortem`
